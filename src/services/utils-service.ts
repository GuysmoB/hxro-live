...
export class UtilsService {
  ...
  obUpdate(buffer: any[][], snapshot: any[][]) {
    ...
    const bidLimitDepthPrice = price - (price * (5 / 100));
    const askLimitDepthPrice = price + (price * (5 / 100));
 ...
 */
   ...
   */
  async updateFirebaseResults(databasePath: string, _candle: any) {
    try {
      const res = await this.getFirebaseResults(databasePath);
      if (res) {
        await firebase.database().ref(databasePath).push({ candle: _candle });
      }
    } catch (error) {
